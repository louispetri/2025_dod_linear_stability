...
for i in range(0, numberOfErrors):
    ...
    for j in range(0, m):
        ...
        L2Error[j, i] = errorData[i]['L2Error'][j]

current_palette = sb.color_palette()

fig, ax = plt.subplots(nrows=3, ncols=m, squeeze=False)
...
for i in range(0, m):
    ...
    L2Order = np.polyfit(np.log(N), np.log(L2Error[i, :]), 1)

    ax[1, i].plot(N, L2Error[i, :],linestyle='dotted', marker='o',label="error", color=current_palette[0])
    ax[1, i].plot(N, np.exp(L2Order[1]) * N**(L2Order[0]), label="order {:.4f}".format(-L2Order[0]), color=current_palette[0])
    ax[1, i].plot(N, np.exp(L2Order[1]) * N[0]**expectedOrder * N[0]**L2Order[0] * N**(-expectedOrder), label="order {:.4f}".format(expectedOrder), color=current_palette[1])

    ax[1, i].set_yscale('log')
    ax[1, i].set_xscale('log')

    ax[1, i].set_box_aspect(1)
    ax[1, i].set_title(f'$L^2$-error $u_{i}$')
