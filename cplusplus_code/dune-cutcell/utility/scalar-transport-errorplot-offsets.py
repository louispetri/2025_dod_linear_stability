...
errorFiles = glob.glob('offset=*/error*')
...
errorData.sort(key = lambda error : (error['offset']))
...
while startIndex != endIndex:
    ...
    for i in range(startIndex, currentIndex):
        N[i - startIndex] = errorData[i]['offset']
    ...
    ax[0, 1].plot(N, LInfError[0, :],linestyle='dotted', marker='o',label=labelText + space + " {:.4f}".format(-LInfOrder[0]), color=color)
...
ax[0, 1].set_title(f'Convergence L-$\\infty$ norm error')
