...
using LinearAlgebra, PolynomialBases, LaTeXStrings, SummationByPartsOperators, DelimitedFiles
...
for basis in [LobattoLegendre, GaussLegendre]
    for order in [1,2,3,4]
        @time steps, errors = convergence_error_comparison(basis, order; exprange = [6,12], sec_factor = 0.99, alignCFL = false, Tmax = 3.0)
        ...
        writedlm(joinpath(@__DIR__,"./conv_work_precision_data/$(basissstr)_order=$(order)_data.txt"), data)
    end
end